...
def coordinate_feature(mesh):
    features = np.zeros((5, mesh.edges_count))  
   
    for edge_id in range(mesh.edges_count):
        main_edge_vertices = mesh.vs[mesh.edges[edge_id]]
        main_edge_midpoint = np.mean(main_edge_vertices, axis=0)
        features[0, edge_id] = np.mean(main_edge_midpoint)


        for neighbor_idx in range(4): 
            neighbor_edge_id = mesh.gemm_edges[edge_id, neighbor_idx]
            if neighbor_edge_id != -1:
                neighbor_edge_vertices = mesh.vs[mesh.edges[neighbor_edge_id]]
                neighbor_midpoint = np.mean(neighbor_edge_vertices, axis=0)
                features[neighbor_idx + 1, edge_id] = np.mean(neighbor_midpoint)
            else:
                features[neighbor_idx + 1, edge_id] = 0
    return features
...
def extract_features(mesh):
    ...
    with np.errstate(divide='raise'):
        try:
            ...
            feature = coordinate_feature(mesh)
            return feature
